...
untrusted (or secret) data and then retypes the same content as a fresh
literal in a later statement: found live, first against qwen2.5:32b in
the AgentDojo integration, where a poisoned bill's IBAN got typed
verbatim into send_money(recipient="...") as a plain string literal,
never a reference to the tainted variable holding the bill's content.
A literal is TRUSTED/PUBLIC by construction in eval_node's ast.Constant
case -- there is no data-flow edge for any taint tracker, this one
included, to check.
...
on its own has no reason to exactly match content it never typed itself
-- but this is a runtime string-matching heuristic with a real
...
Deliberately kept out of interpreter.py: the core module's own docstring
is explicit that the whitelist boundary is about what's allowed to run,
not about catching every mistake a legal operation can still make.
"""
...
class RetypingDetected(Exception):
    ...
    Not raised by RetypingGuard itself -- check_statement always returns
    flags rather than raising, so a caller can choose to log, route to a
    human, or block, per notes/PRODUCTION_ROADMAP.md item 12's three
    proposed responses. enforce() is the convenience path for the
    blocking response."""
...
def _extract_strings(value: Any, _seen: set[int] | None = None) -> list[str]:
    ...
    tuple, set, dict, or a plain object's attributes (the same shape
    real tools return -- e.g. AgentDojo's Transaction objects, which is
    exactly why interpreter.py's ast.Attribute case exists). Deliberately
    permissive: missing a nested string only weakens detection, it never
    breaks anything, so this errs toward finding more text rather than
    less. _seen guards against a value that references itself, which a
    real tool's return value should never do but a hostile or buggy one
    could."""
    if _seen is None:
        _seen = set()
...
class RetypingGuard:
    ...
    `privileged`/`sinks` call that overlaps it. One guard per run/session
    -- like Session's env and budget, a guard's tracked text is specific
    to one task and must never be shared across independent runs."""

    def __init__(self, min_length: int = 6):
        """min_length excludes short matches (a single digit, a common
        word) that would otherwise false-positive constantly -- chosen
        as a starting point, not measured against real traffic yet;
        notes/PRODUCTION_ROADMAP.md item 12 says as much needs a real
        false-positive-rate measurement before being trusted for
    ...
    def check_statement(
        ...
        contract, but also works on a full multi-statement program the
        way run() takes one) and returns a RetypingFlag for every
...
def enforce(flags: list[RetypingFlag]) -> None:
    ...
    response from notes/PRODUCTION_ROADMAP.md item 12's three proposals
    -- a caller that instead wants the "coarser human-in-the-loop" or
    "provenance trace annotation" response should inspect flags directly
...
def run_turn_guarded(
    ...
    *,
    ...
    first and raises RetypingDetected instead of executing it if
    anything is flagged -- turns RetypingGuard from an observe-only
    layer into an actual block, closing the gap rather than just
    reporting it. `allowed` must already be wrapped with
    ...
    An AST rewrite that replaces a retyped literal with a reference to
    the variable holding the real value (turning the statement into
    ...
    substring of it (the IBAN inside the bill) -- not equal to the
    variable's value, so there is no whole-variable reference to
    substitute in. Building the correct replacement would mean
    constructing a substring/slice expression, and this grammar has no
    string indexing or slicing to write one in (see
    notes/ROADMAP.md item 1's grammar remainders). Blocking, using the
    substring match check_statement() already does correctly, is the
    mechanism that actually closes the case that was found -- the
    rewrite idea is recorded here rather than silently dropped, since
    it's the kind of thing worth not re-discovering the hard way twice.
    """
...
def wrap_for_retyping_guard(
    ...
    confidential entries are wrapped to also record their return value
    with guard. Doesn't modify allowed in place, matching the rest of
    this codebase's preference for not mutating a caller's dict out from
    under it. A name in both sources and confidential gets recorded to
    both buckets -- matching interpreter.py's own precedent that sources
    wins over sanitizers and confidential wins over declassifiers when a
    configuration lists a name in two roles, this doesn't need a
    tie-break at all, since recording to both buckets is harmless (the
    only effect is more text tracked, not a conflicting decision)."""
